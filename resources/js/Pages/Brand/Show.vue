<template>
    <AppLayout title="Danh sách nhãn hàng">
        <template #header>
            ...
            <div class="max-w-12xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg pt-8 pb-12 px-2">
                    ...
                    <div class="float-right text-xs uppercase">
                        <Link :href="route('Brand.trash')" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-2 rounded mr-4">
                        <icon :icon="['fas', 'trash']" class="mr-3" />Thùng rác ({{ trashCount }})
                        </Link>
                        <Link :href="route('Brand.create')" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-2 rounded ">
                        ...
                        <DataTable :headers="headers" :items="dataPage" buttons-pagination show-index v-model:items-selected="itemsSelected">
                            ...
                            <template #item-operation="{ id, name }">
                                <div class="py-3 flex items-center justify-center">
                                    ...
                                    <Link :href="route('Brand.edit', id)" class="bg-yellow-600 text-white px-2 py-1 rounded-md mr-5">
...
export default {
    ...
    setup() {
        ...
        const loadData = () => {
            axios.post('brand/load-data-table')
                .then((response) => {
