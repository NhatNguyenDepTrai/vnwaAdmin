            ...
            <div class="max-w-12xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg pt-8 pb-12 px-2">
                    ...
                    <div class="float-right text-xs uppercase">
                        <Link :href="route('Blog.trash')" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-2 rounded mr-4">
                        <icon :icon="['fas', 'trash']" class="mr-3" />Thùng rác ({{ trashCount }})
                        </Link>
                        <Link :href="route('Blog.create')" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-2 rounded ">
                    ...
                    <div class="my-2 py-10">
                        <DataTable :headers="headers" :items="dataPage" buttons-pagination show-index v-model:items-selected="itemsSelected">
                            ...
                            <template #header-nameCategoryBlog="header">
                                <div class="filter-column  flex items-center">
                                    <div>
                                        <button class="p-2 text-center  mr-2 border-none " :class="{ 'bg-purple-400 text-white': search.category_Blog.trim() }" @click="appear.category_Blog = !appear.category_Blog">
                                            <icon :icon="['fas', 'filter']" />
                                        </button>
                                        <div class="filter-menu absolute z-30 top-9 w-52 flex items-center justify-center" v-if="appear.category_Blog">
                                            <input style="height: 30px;" type="text" class="text-xs h-8 border-r-0" v-model="search.category_Blog" @input="searchDataTable('category_Blog', $event)" placeholder="Tìm kiếm" />

                                            <button style="height: 30px; width: 30px;" class="bg-black text-white hover:text-red-400" @click="appear.category_Blog = false">
                                                <icon :icon="['fas', 'x']" />
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <template #item-tags="{ tags }">
                                <div class="py-3 flex items-center justify-start flex-wrap">
                                    <div class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300" v-for="(item, index) in tags" :key="index">{{ item.name }}</div>
                                </div>
                            </template>
                            ...
                            <template #item-operation="{ id, name }">
                                <div class="py-3 flex items-center justify-center">
                                    ...
                                    <Link :href="route('Blog.edit', id)" class="bg-yellow-600 text-white px-2 py-1 rounded-md mr-5">
...
export default {
    ...
    data() {
        return {
            ...
            itemsSelected: [],
            headers: [
                { text: "Danh mục", value: "nameCategoryBlog" },
    ...
    setup() {
        ...
        const loadData = () => {
            axios.post('Blog/load-data-table')
                .then((response) => {
        ...
        const appear = ref({ name: false, category_Blog: false });
        const search = ref({ name: '', category_Blog: '' });
